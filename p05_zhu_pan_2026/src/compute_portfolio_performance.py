...
def _merge_intraday(intraday_headlines):
    ...
    daily_stock = pd.read_parquet(DATA_DIR / "clean" / "crsp_daily.parquet")
    taq = pd.read_parquet(DATA_DIR / "taq_nbbo_minute.parquet") # for intraday prices
    ...
    intraday_headlines_merged['drift'] = (intraday_headlines_merged['next_close'] - intraday_headlines_merged['closeprc']) / intraday_headlines_merged['closeprc']
...
def calculate_portfolio_metrics(
    ...
    return {
        "portfolio": portfolio,
        "firm_day_observations": int(len(df)),
