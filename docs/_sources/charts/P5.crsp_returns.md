---
date: 2026-02-08 01:38:20
tags: CRSP
category: 
---

# Chart: CRSP Daily Returns
Simple exploratory time series of daily stock returns from CRSP

## Chart
```{raw} html
<iframe src="../_static/P5/crsp_returns.html" height="500px" width="100%"></iframe>

<p style="text-align: center;">Sources: CRSP</p>
```
[Full Screen Chart](../download_chart/P5/crsp_returns.html)





Exploratory Plotly time-series chart of daily CRSP returns, saved as an interactive HTML file.


## Chart Specs

| Chart Name             | CRSP Daily Returns                                             |
|------------------------|------------------------------------------------------------|
| Chart ID               | crsp_returns                                               |
| Topic Tags             |                                 |
| Data Series Start Date |                                  |
| Data Frequency         |                                          |
| Observation Period     |                                      |
| Lag in Data Release    |                                     |
| Data Release Timing    |                                     |
| Seasonal Adjustment    |                                     |
| Units                  |                                                   |
| HTML Chart             | [HTML](../download_chart/P5/crsp_returns.html)    |


## Dataframe Manifest

| Dataframe Name                 |                                                    |
|--------------------------------|--------------------------------------------------------------------------------------|
| Dataframe ID                   | [crsp_daily](../dataframes/P5/crsp_daily.md)                                       |
| Data Sources                   | CRSP                                        |
| Data Providers                 | WRDS                                      |
| Links to Providers             |                              |
| Topic Tags                     |                                           |
| Type of Data Access            |                                   |
| How is data pulled?            | Pulled via wrds python package in src/pull_CRSP_stock.py                                                    |
| Data available up to (min)     |                                                              |
| Data available up to (max)     |                                                              |
| Dataframe Path                 | E:\full stack\p05_zhu-pan_2026\p5\_data\crsp_daily.parquet                                                   |


**Linked Charts:**


- [P5:crsp_returns](../../charts/P5.crsp_returns.md)



## Pipeline Manifest

| Pipeline Name                   | Can ChatGPT Forecast Stock Price Movements? Return Predictability and Large Language Models                       |
|---------------------------------|--------------------------------------------------------|
| Pipeline ID                     | [P5](../index.md)              |
| Lead Pipeline Developer         | samoszhu             |
| Contributors                    | samoszhu           |
| Git Repo URL                    |                         |
| Pipeline Web Page               | <a href="file://E:/full stack/p05_zhu-pan_2026/p5/docs/index.html">Pipeline Web Page      |
| Date of Last Code Update        | 2026-02-08 01:38:20           |
| OS Compatibility                |  |
| Linked Dataframes               |  [P5:crsp_daily](../dataframes/P5/crsp_daily.md)<br>  |

